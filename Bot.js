...
function saveData() {
    try {
        fs.writeFileSync(DATA_FILE, JSON.stringify({ laporan, antrian }, null, 2));
    } catch (e) {
        console.log("⚠️ Gagal save data:", e.message);
    }
}

let { laporan, antrian } = loadData();
let userState = {};
let verifiedUsers = {};
let loginState = {};

const token = '8718779525:AAEIpND93CZhL4wJFfHpMjaiBNpRB01fQ4c';
...
function jadwalReset() {
    ...
    setTimeout(() => {
        ...
        fs.writeFileSync(backupFile, JSON.stringify({ laporan, antrian }, null, 2));
        laporan = [];
        antrian = [];
        userState = {};
        saveData();
...
function showMenu(chatId) {
    ...
    if (role === "admin") {
        menu.push(["📤 Export Pengawasan Jalur ROW"]);
        menu.push(["📊 Dashboard Pengawasan Jalur ROW"]);
...
bot.on('message', async (msg) => {
    ...
    if (text === "📋 History Pengawasan") {
        ...
        if (dataUser.length === 0) return kirimPesan(chatId, "❌ Belum ada history pengawasan");
    ...
    // DASHBOARD
    // ==================
    if (text === "📊 Dashboard Pengawasan Jalur ROW") {
        ...
        let infoAman = Object.keys(segAman).length === 0 ? "   Tidak ada data"
            : Object.keys(segAman).map(s => {
                return `   • ${s} (${segAman[s].length} titik)`;
            }).join("\n");
        ...
        let infoBersemak = Object.keys(segBersemak).length === 0 ? "   Tidak ada data"
            : Object.keys(segBersemak).map(s => {
                return `   • ${s} (${segBersemak[s].length} titik)`;
            }).join("\n");
...
`📊 DASHBOARD PENGAWASAN JALUR ROW

📏 Total Pengawasan KP Jalur ROW : ${totalMeter} meter
...
${infoPelanggaran}`);
    ...
    if (text === "👷 Data Petugas Jalur ROW") {
        if (verifiedUsers[chatId].role !== "admin") return kirimPesan(chatId, "❌ Akses ditolak");
        if (laporan.length === 0) return kirimPesan(chatId, "Belum ada data petugas");

        let rekap = {};
        laporan.forEach(d => {
            if (!rekap[d.user]) rekap[d.user] = { totalTitik: 0, segments: {} };
...
`📷 Kirim foto untuk KP ${formatKP(userState[chatId].kp)}\n\n• 3 foto = ✅ Jalur Aman\n• 4 foto = 🌿 Bersemak\n• 5 foto = 🚧 Ada Finding/Pelanggaran`);
    ...
    if (text === "📤 Export Pengawasan Jalur ROW") {
        if (laporan.length === 0) return kirimPesan(chatId, "❌ Belum ada data");
        ...
        if (role === "petugas") {
            ...
            if (dataKu.length === 0) return kirimPesan(chatId, "❌ Belum ada data pengawasanmu");

            await kirimPesan(chatId, `⏳ Membuat file Excel milik ${nama}...`);
            await exportExcelSatuPetugas(chatId, nama);

        } else if (role === "admin") {
            let daftarPetugas = [...new Set(laporan.map(d => d.user))];
...
bot.on('photo', async (msg) => {
    ...
    await kirimPesan(chatId, "⏳ Menyimpan foto...");
    ...
    if (count === 3) {
        return kirimPesan(chatId, `📸 3 foto diterima\n\nPilih tindakan:`, {
            reply_markup: {
                keyboard: [
    ...
    if (count === 4) {
        return kirimPesan(chatId, `📸 4 foto diterima\n\nPilih tindakan:`, {
            reply_markup: {
                keyboard: [
    ...
    if (count >= 5) {
        return kirimPesan(chatId, `📸 5 foto diterima\n\nPilih tindakan:`, {
            reply_markup: {
                keyboard: [
...
async function buatExcelPetugas(petugas, tanggal) {
    ...
    for (let idx = 0; idx < dataPertugas.length; idx++) {
        ...
        // Embed foto langsung tanpa resize
